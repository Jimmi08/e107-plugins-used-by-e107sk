...
class Canonical  { 
    ...
    function runFixConstants() {
		...
		function getManualCanonicalUrl($requestedUrl = '')  {
		  $sql = e107::getDb();
		...
		function getRelatedCanonicalUrl($element_type = '', $element_id = 0, $backslash = true, $p = 0, $return = false )  {
		  $sql = e107::getDb();         
		  if($element_type == '')  { return false; }    // TODO check allowed types
        ...
        if($row['can_redirect'])  {
          ...
          exit;
		 	  ...
		 	  // add backslash to canonical url
			  if($backslash) {
