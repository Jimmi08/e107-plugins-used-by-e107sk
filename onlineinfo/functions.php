...
{ // Fuction to get User Colour
	...
	if ($pref['onlineinfo_onoffcolour'] == 1)
	{
		...
		// FIX SPEED   added  user_class <> ""

		$where = 'WHERE user_id="' . $userid . '"   ';
		if ($row = $sql->retrieve('user', 'user_class,user_admin,user_perms', $where))
		{
		}
		else return '';
		$usermodr = 0;
		...
		for ($z = 0; $z <= count($userc); $z++)
		{
			...
			$row2 = $sql->retrieve('forum', 'forum_moderators', " forum_parent <> '0' ");
			if ($userc[$z] == $row2[0])
			{
				$usermodr = 1;
			}
		...
		if ($pref['onlineinfo_adminactive'] == 1)
		{
			if ($user_admin == 1)
			{
				$usercol = 'style="text-decoration: none; font-weight:bold; color:' . $pref['onlineinfo_admincolour'] . ';"';
			}
		}
		...
		if ($pref['onlineinfo_headadminactive'] == 1)
		{
			...
			{
				$usercol = 'style="text-decoration: none; font-weight:bold; color:' . $pref['onlineinfo_headadmincolour'] . ';"';
			}
		}

		if ($pref['onlineinfo_modactive'] == 1)
		{
			if ($usermodr == 1 && $user_admin <> 1)
			{
				$usercol = 'style="text-decoration: none; font-weight:bold; color:' . $pref['onlineinfo_modcolour'] . ';"';
			}
		}

		if ($pref['onlineinfo_memactive'] == 1)
		{
			if ($usermodr == 0 && $user_admin <> 1)
			{
				$usercol = 'style="text-decoration: none; color:' . $pref['onlineinfo_memcolour'] . ';"';
			}
		}

		if ($usercol == "" || $usercol == 'style="text-decoration: none; color:' . $pref['onlineinfo_memcolour'] . ';"')
		{ // user classes
...
if($layout==1){
	$colourkey='<div style="text-align:left;" class="mediumtext"><img src="'.e_PLUGIN_ABS.'onlineinfo/images/keys.png" height="18px" /><b>'.ONLINENOW_7.'</b>:</div>';
...
$pdata = eregi_replace('comment.comment', 'Comment ', $pdata);			
		 */
$pdata = preg_replace('/flashchat/i',ONLINEINFO_LOGIN_MENU_L62, $pagedata);
$pdata = preg_replace('/getxml/i',ONLINEINFO_LOGIN_MENU_L62, $pdata);									
$pdata = preg_replace('/sig/i',ONLINEINFO_LOGIN_MENU_L71, $pdata);
$pdata = preg_replace('/bf2142BF2 Sig Gen/i',ONLINEINFO_LOGIN_MENU_L85, $pdata);
$pdata = preg_replace('/create_sig Gen/i',ONLINEINFO_LOGIN_MENU_L85, $pdata);
$pdata = preg_replace('/BF2 Sig Gennup/i',ONLINEINFO_LOGIN_MENU_L3, $pdata);
$pdata = preg_replace('/bf2stats/i',ONLINEINFO_LOGIN_MENU_L72, $pdata);
$pdata = preg_replace('/rss/i',ONLINEINFO_LOGIN_MENU_L73, $pdata);
$pdata = preg_replace('/ratepic/i',ONLINEINFO_LOGIN_MENU_L74, $pdata);
//$pdata = eregi_replace('thumbnails',ONLINEINFO_LOGIN_MENU_L74, $pdata);
$pdata = preg_replace('/displayimage/i',ONLINEINFO_LOGIN_MENU_L74, $pdata);
$pdata = preg_replace('/ecard/i',ONLINEINFO_LOGIN_MENU_L74, $pdata);
$pdata = preg_replace('/addfav/i',ONLINEINFO_LOGIN_MENU_L74, $pdata);					                
$pdata = preg_replace('/comment.comment.news./i', ONLINE_EL12. ' - ', $pdata);
$pdata = preg_replace('/comment.comment.poll./i', ONLINEINFO_LOGIN_MENU_L63, $pdata);
$pdata = preg_replace('/comment.comment/i', 'Comment ', $pdata);	
