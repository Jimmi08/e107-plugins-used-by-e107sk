...
if (check_class($extraclass) && e107::isInstalled('forum'))
{
    ...
    LIMIT 0," . $extrarecords . "";     
     /*
		$query = "
		SELECT COUNT(ft.thread_id) AS ucount, u.user_name, u.user_id FROM #forum_thread as ft
		LEFT JOIN #user AS u ON ft.thread_user = u.user_id
		GROUP BY ft.thread_user
		ORDER BY ucount DESC LIMIT 0,10";
	...
	if ($extraacache == 1)
	{
		...
		$currenttime = time();
		$script = "SELECT * FROM #onlineinfo_cache Where type='toppoststarter'";
		$sql->gen($script);
		while ($row = $sql->fetch())
		{
			extract($row);
