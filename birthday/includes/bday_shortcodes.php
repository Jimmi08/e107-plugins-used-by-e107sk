...
SC_BEGIN BDAY_LOGO
global $BDAY_today;
    if ($BDAY_today && is_readable(e_PLUGIN . "birthday/images/bdayanimate.gif"))
    {
        return   "<img src='" . e_PLUGIN_ABS . "birthday/images/bdayanimate.gif' alt='" . BDAY_LAN_7 . "' title='" . BDAY_LAN_8 . "' /><br />";
    }
    else
    {
    	return '';
...
{
$name = $tp->toHTML($user_name, false);
$url = e107::url('user/profile/view', array('name' => $name, 'id' => $user_id));
...
else
{
  ...
  return $bday_out . " <a title='" . $user_birthday = "$BDAY_datepart[2].{$BDAY_datepart[1]}.{$bday_showyear}" . "' 
  href='" . $url .  "'>" . $tp->toHTML($user_name, false) .  "</a>";

}
SC_END

SC_BEGIN BDAY_UPDATE
global $user_id,$tp,$bday_show,$bday_out;
$plugPref =  e107::pref('birthday');
if($plugPref['bday_showdate'] == 1)
{
	if($parm=='nolink')
	{
		return $bday_out;
...
{
return '<a href="'.e_PLUGIN_ABS.'birthday/bday_ages.php" >'.BDAY_LAN_9.'</a>';
}
else
{
return '';
