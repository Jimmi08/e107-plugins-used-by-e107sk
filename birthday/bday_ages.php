...
$sql = e107::getDb();
$corePrefs = e107::pref('core', 'bday_dformat');
...
if (!check_class($plugPref['bday_demographic']))
{
...
else
{
	...
	// loading templates this way is intention

	if (file_exists(THEME . 'templates/birthday/bday_age_template.php'))
	{
		define('BDAY_ATEMPLATE', THEME . 'templates/birthday/bday_age_template.php');
	}
	else
	{
		define('BDAY_ATEMPLATE', e_PLUGIN . 'birthday/templates/bday_age_template.php');
	}
	...
	$sql->gen('select count(user_extended_id) as bday_noentry  from #user_extended left join #user  on user_extended_id = user_id 
	where (user_birthday="0000-00-00" OR user_birthday IS NULL) and user_id>0', false);
	extract($sql->fetch());
...
FROM #user_extended left join #user on user_extended_id = user_id where user_id>0 GROUP BY age ;", false);
	...
	foreach($bday_result as $key => $value)
	{
		...
		$bday_bar = "<img src='" . e_PLUGIN . "birthday/images/box{$bday_anal}.png' style='border:0px;' alt='{$bday_percent}%' title='{$bday_percent}%' />";
