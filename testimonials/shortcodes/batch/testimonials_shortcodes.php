...
class testimonials_shortcodes extends e_shortcode
{
	 ...
	 * @return string
	 */
	function sc_testimonials_items($parm)
	{
        $template_key = varset($this->var['template_key'], 'menu');
		$template = e107::getTemplate('testimonials', null, $template_key );
		...
		$text = '';      
		foreach($group as $cnt)
		{                            
			$this->var = $cnt;
			$text .= e107::getParser()->parseTemplate($template['item'],true, $this);
		}
	...
	function sc_testimonials_message()
	{
		...
		else
		{
			...
			//fix display expand button only if it is needed
      if($remainder) {
    ...
    //{TESTIMONIALS_URL}
	function sc_testimonials_url()
	{
		return $this->var['tm_url'];
	}
	 ...
	 *
	 * @return string
	 * {TESTIMONIALS_AUTHOR: type=name}
	 */
	function sc_testimonials_author($parm=array())
	{
 
		if($parm['type'] == 'name') {
		    return $this->var['tm_name'];
		}
		...
		if($uid === 0)
		{
			return !empty($this->var['user_name']) ? $this->var['user_name'] : $this->var['tm_name'];
		}
	...
	/**
	 * Testimonial author's position. 
	 *
	 * @return string
	 */
	function sc_testimonials_position()
	{
		if(!empty($this->var['tm_position']))
		{
			return $this->var['tm_position'];
		} 
 	}
	 ...
	 * @return mixed
	 * {TESTIMONIALS_IMAGE}
	 * {TESTIMONIALS_IMAGE: url=1}
	 * {TESTIMONIALS_IMAGE: force=1&placeholder=1}
	 * {TESTIMONIALS_IMAGE: url=1&size=150x150}
	 */
	function sc_testimonials_image($parm=array())
	{
 
		$tp = e107::getParser();
    if(!empty($this->var['tm_image'])) {
		 $imagepath = $this->var['tm_image'];
		}
		elseif(!empty($this->var['user_image'] && $parm['use_user'] = 1)) {
		  $imagepath = $this->var['user_image'];
		}
		else {
		  $imagepath = e_PLUGIN_ABS."testimonials/images/testi-thumb-".varset($parm['size'], "100x100").".jpg";
		}
		
		$parm['alt'] = $this->var['tm_name'];
		$parm['class'] = varset($parm['class'],"none");
    return $tp->toImage($imagepath, $parm); 
	}
	
	function sc_testimonials_subtext()
	{
		if($this->plugPrefs['tm_menu_subtext'])
		{
			$tp = e107::getParser();
			return $tp->toHTML($this->plugPrefs['tm_menu_subtext'], 'BODY');;
		}  
	}
    ...
    /* {TESTIMONIALS_RATING} */
	function sc_testimonials_rating($parm='') 
	{
		$pref = e107::getPref();
		$frm = e107::getForm();
		$template_key = varset($this->var['template_key'], 'menu');
    	...
    	if( (int)$this->var['tm_rating'] > 0)
		{
			$num = $this->var['tm_rating'];
			for($i=1; $i<= $num; $i++)
			{
