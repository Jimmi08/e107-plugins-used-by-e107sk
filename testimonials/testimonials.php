...
class testimonials
{
	...
	function formSubmit()
	{
		...
		$insert = array(
			...
			'tm_ip'        => $ip,
			'tm_order'     => 0,
			'tm_position'   => $tp->filter($_POST['tm_position'])
		);
	...
	function renderPage()
	{
		...
		$tm_position = (isset($_POST['tm_position']) ? $_POST['tm_position'] : '');

		$form .= '<div class="form-group">';
		$form .= $frm->text('tm_position', $tm_position, 50, array(
			'id'          => 'tm_position',
			'class'       => 'form-control tbox span12',
			'placeholder' => "Position: eg. CEO of MyCompany.Inc",
		));
		$form .= '</div>';
