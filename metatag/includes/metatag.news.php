...
function metatag_entity_news_list_detect()
{
	...
	if($page == 'news.php'  && substr($query, 0, 8) == 'default.')
	{
		return true;
	}
 ...
 */
function metatag_entity_news_token_category_canurl($entity)
{   
    $page = (int) $_GET['page'] ;
	if($page > 1)  {
	    $p = '?page='.$page;
	}
    $id =  varset($entity['category_id'], '');
    if($id) 
    {  
      if(e107::isInstalled('jm_canonical')) { 
		  e107_require_once(e_PLUGIN . 'jm_canonical/canonical.class.php');
	      $canonicalPlugin = new Canonical;  
	      $result = $canonicalPlugin->getRelatedCanonicalUrl("news_category", $id, true,  $p, true); 
	      if($result) {
	        return $result;
	      }
	      else {
	         ...
	         if($pluginPrefs['news_auto'] != "none") {
	          //autogeneration    $table, $type, $backslash, $return
	          return $canonicalPlugin->getAutogeneratedCanonicalUrl("news_category", $pluginPrefs['news_auto'], $pluginPrefs['backslash'], true);
 ...
 * Returns with Canonical URL.
 */
function metatag_entity_news_token_canurl($entity)
{   
    ...
    { 
		...
		{ 
			e107_require_once(e_PLUGIN . 'jm_canonical/canonical.class.php');
			$canonicalPlugin = new Canonical;  
			$result = $canonicalPlugin->getRelatedCanonicalUrl("news", $id, true,  $p, true); 
			if($result) {      
				return $result;
			}
			else 
			{             
				...
				if($pluginPrefs['news_auto'] != "none") {      
					//autogeneration
					return $canonicalPlugin->getAutogeneratedCanonicalUrl("news", $pluginPrefs['news_auto'], $pluginPrefs['backslash'], true);
				}
