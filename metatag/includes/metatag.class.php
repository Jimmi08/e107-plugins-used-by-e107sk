...
class metatag
{
    ...
    $this->jmcorePrefs =  e107::getPlugConfig('metatag')->getPref();
	...
	public function getWidget($values = array(), $field = 'x_metatag_metatags')
	{
		...
		// Render token info and button.
		    //displaye available tokens in view, not on button
    $globalTokens =  $this->widgetTokens['token']['global_tokens']['tokens'];
    $entityTokens =  $this->widgetTokens['token']['entity_tokens']['tokens'];
  
    foreach($globalTokens as $token)  {
		 $tokensglobal .= "{".$token['token']."} ";    
		}
    foreach($entityTokens as $token)  {
		 $tokensentity .= "{".$token['token']."} ";    
		}
    
		$sc->setVars(array(
			'token_help'   => '<h4>' . LAN_PLUGIN_METATAG_HELP_06 . '</h4><p>' . LAN_PLUGIN_METATAG_HELP_01 . '</p>',
			'token_global' =>  $tokensglobal,	
			'token_entity' =>  $tokensentity,				
	  ...
	  $html .= $tp->parseTemplate($tpl['TOKEN'], true, $sc);
   	...
   	if($this->jmcorePrefs['metatag_title']) {
   ...
   if($this->jmcorePrefs['metatag_description']) {
   ...
   if($this->jmcorePrefs['metatag_abstract']) {
   ...
   if($this->jmcorePrefs['metatag_keywords']) {    
    ...
    if($this->jmcorePrefs['metatag_canonical']) {  
		 $basic[$field . '[canonical]'] = array(
   ...
   if($this->jmcorePrefs['advanced_rating']) {
    ...
    if($this->jmcorePrefs['advanced_referrer']) { 
   ...
   if($this->jmcorePrefs['metatag_dublin']) {
		...
		// Google+ meta tags.
		$google = array();
    if($this->jmcorePrefs['metatag_googleplus']) {
	...
	public function renderMeta($name, $content)
	{
		...
		{  
		  e107::meta(null, $content, array( 'name' => $name));
      
      if($name == "description") { 
       e107::getSingleton('eResponse')->addMetaDescription($content); 
      }  
      if($name == "title") { 
      e107::getSingleton('eResponse')->addMetaTitle($content); 
      }
       if($name == "keywords") {  
      e107::getSingleton('eResponse')->addMetaKeywords($content); 
      }       
		}
	}
